...
class Tx_T3tt_Domain_Model_Data_Transformator {
	...
	/**
	 * @var Tx_T3tt_Domain_Model_Request_DataTransformationRequest
	 */
	...
	/**
	 * @var Tx_T3tt_Domain_Service_AbstractXsltProcessor
	 */
	...
	private function execPhpHooks($category) {
		...
		$output = NULL;
		foreach ($this->_phpHookAggregation[$category] as $func) {
			$output = $func($lines);
		}
	...
	private function execXsltTransformation() {
		$stylesheetDomObj = new DOMDocument();
		$stylesheetDomObj->loadXML($this->_xsltStylesheet);
		if (!$stylesheetDomObj) {
		...
		$dataDomObj = new DOMDocument();
		$dataDomObj->loadXML($this->_data);
		if (!$dataDomObj) {
